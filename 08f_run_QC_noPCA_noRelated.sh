...
#no need to check independent individuals, they are removed from the dataset
#no need to set parental info to zero, they are already missing
#rename our dataset to match the script by adding .preprocessed
cp ${fname}.fam ${fname}.preprocessed.fam
cp ${fname}.bed ${fname}.preprocessed.bed
cp ${fname}.bim ${fname}.preprocessed.bim
...
${xwasloc}/$xwasQ --bfile ${fname} --make-bed --out ${fname}_qc2 --exclude pars.txt --range
...
#    ${xwasloc}/$xwasQ --bfile ${fname}_qc2 --make-bed --out ${fname}_qc4 --remove ${fname}_exclind.remove
#else
#    echo -e "6: Skipping IBD analysis"
#    ${xwasloc}/$xwasQ --bfile ${fname}_qc2 --make-bed --out ${fname}_qc4
#fi
...
if [ $malenum -eq 0 ] || [ $femalenum -eq 0 ]; then
	...
	echo "8.1: HWE"
	${xwasloc}/$xwasQ --bfile ${fname}_qc2 --hardy --out ${fname}_1gender_hwe
	...
	${xwasloc}/$xwasQ --bfile ${fname}_qc2 --make-bed --out ${fname}_qc5 --mind ${mindthresh} --maf ${minmaf} --geno ${genothresh} --exclude ${fname}_1gender_snp.exclude
    else
	echo -e "8: Quality Control for Binary Traits"

	echo "8.1: HWE"
	${xwasloc}/$xwasQ --bfile ${fname}_qc2 --hardy --out ${fname}_1gender_hwe --filter-controls
	cat ${fname}_1gender_hwe.hwe | grep ALL | awk -v bf=$bonf '$9<bf {print $2}' > ${fname}_1gender_snp.exclude

	echo "8.2: Correlation between missingness and phenotype"
	${xwasloc}/$xwasQ --bfile ${fname}_qc2 --test-missing --out ${fname}_1gender_mcc
	...
	${xwasloc}/$xwasQ --bfile ${fname}_qc2 --filter-males --make-bed --out ${fname}_male
	...
	${xwasloc}/$xwasQ --bfile ${fname}_qc2 --filter-females --make-bed --out ${fname}_female
...
if [ "$savemethod" = "none" ]; then # remove all intermediate files
    ...
    rm ${fname}_sex* pars.txt  temp_params.txt
    if [ $skipibd -eq 0 ]; then
	rm ${fname}_rel*
    fi
...
#    mv ${old_fname}.PC* ${fname}.bed ${fname}.bim ${fname}.fam $INTER/
    ...
    mv ${fname}_qc* $INTER/
    mv ${fname}_sex*  pars.txt temp_params.txt $INTER/
    if [ $skipibd -eq 0 ]; then
	mv ${fname}_rel* $INTER/
    fi
fi
