...
$PLINK --bfile $DATA/met_better_cluster4 \
       ...
       --covar-name sen.better.rank, age-ePC10 \
       --out $GWAS/met_better_cluster4_gwas

$PLINK --bfile $DATA/sen_better_cluster4 \
       --linear  --ci 0.95 --covar $COVAR/covar_euro.txt \
       --covar-name met.better.rank, sex-ePC10\
       --out $GWAS/sen_better_cluster4_gwas
...
#sort the data
for phenotype in "met_better" "sen_better" "met_better_cluster4" "sen_better_cluster4" "met_better_males" "met_better_females" "sen_better_males" "sen_better_females" 
do
	sort -g -k12 $GWAS/${phenotype}_gwas.assoc.linear | awk '$12 != "NA"'|awk '$12 != 0'|awk 'NR==1 || $5 == "ADD"' > $GWAS/${phenotype}_gwas_sorted_noNA_add
