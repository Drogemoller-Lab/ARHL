...
do
		python3.8 $POLYFUN/munge_polyfun_sumstats.py \
			--sumstats $DATA/${phen}_cluster4_lifted_hrm.txt \
			--out $DATA/${phen}_cluster4_munged.parquet \
			--min-info 0 \
			--min-maf 0
		python3.8 $POLYFUN/extract_snpvar.py \
                      --sumstats $DATA/${phen}_cluster4_munged.parquet \
                      --out $DATA/${phen}_cluster4_snps_with_var.gz \
                      --allow-missing
done
...
do
	...
	do
			...
			python3.8 $POLYFUN/create_finemapper_jobs.py\
				--sumstats $DATA/${phen}_cluster4_snps_with_var.gz \
				--n 17531 \
				--pvalue-cutoff 1e-5 \
				--finemap-exe $FINEMAP/finemap_v1.4.1_x86_64 \
				--method ${tool} \
				--max-num-causal 5 \
				--out-prefix ${phen}_cluster4_polyfun_all_${tool} \
				--jobs-file $DATA/${phen}_cluster4_polyfun_all_${tool}_jobs.txt
			#Genome-wide fine-mapping step 2: Invoking the region-specific jobs
			chmod +x $DATA/${phen}_cluster4_polyfun_all_${tool}_jobs.txt
			$DATA/${phen}_cluster4_polyfun_all_${tool}_jobs.txt
			
			#Genome-wide fine-mapping step 3: Aggregating the results
			python3.8 $POLYFUN/aggregate_finemapper_results.py \
				--out-prefix $DATA/${phen}_cluster4_polyfun_all_${tool} \
				--sumstats $DATA/${phen}_cluster4_snps_with_var.gz \
				--out $DATA/${phen}_cluster4_polyfun_${tool}_agg.txt.gz \
				--pvalue-cutoff 1e-5
	done
done
